...
echo "Should return HTTP 201"
echo .
if [ $pauseFlag -eq 1 ]; then read -p "kienboec купил пакет 1-ый раз. Press enter to continue..."; fi


curl -i -X POST http://localhost:10001/transactions/packages --header "Content-Type: application/json" --header "Authorization: Bearer kienboec-mtcgToken" -d ""
echo "Should return HTTP 201"
echo .
if [ $pauseFlag -eq 1 ]; then read -p "kienboec купил пакет 2-ой раз Press enter to continue..."; fi


curl -i -X POST http://localhost:10001/transactions/packages --header "Content-Type: application/json" --header "Authorization: Bearer kienboec-mtcgToken" -d ""
echo "Should return HTTP 201"
echo .
if [ $pauseFlag -eq 1 ]; then read -p "kienboec купил 3-ий пакет. Press enter to continue..."; fi
...
if [ $pauseFlag -eq 1 ]; then read -p "kienboec купил 4-ий пакет. Press enter to continue..."; fi
